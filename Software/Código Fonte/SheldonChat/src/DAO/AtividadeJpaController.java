...
public class AtividadeJpaController implements Serializable {
    ...
    public Atividade findNomeAtividade(String nomeAtividade) {
        EntityManager em = getEntityManager();
        try {
            return em.createNamedQuery("Atividade.findByNomeAtividade", Atividade.class).setParameter("nomeAtividade", nomeAtividade).getSingleResult();
        } catch (Exception e) {
            return null;
        } finally {
            em.close();
